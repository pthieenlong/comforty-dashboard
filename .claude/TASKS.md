...
| Sprint 11 | Audit log + Notifications                  | `[x]`      | `feat/audit-notifications` |
...
**Mục tiêu**: Audit log viewer cho admin (view toàn công ty) + Notification center standalone page.

**Branch**: `feat/audit-notifications` — chia 2 mini-sprint (11a Audit, 11b Notification full page).

### Mini-sprint plan

- **11a** — Audit log: types + mock generator từ existing stores (~400 entry) + AuditStore + functional auditGuard + `/audit` list với filter heavy + detail Drawer field-level diff.
- **11b** — Notification full page: mở rộng mock notification từ 8 lên ~80 entry + `/notifications` standalone page với filter type/read state/date range + bulk action + sidebar entry + topbar dropdown link "Xem tất cả".

### Components cần thêm (Sprint 11)

- [ ] (đã có đủ từ các sprint trước — reuse DataTable/Drawer/DescriptionList/DateRangePicker/Select/Pagination/Tabs/SearchInput)

### Mock data + stores (Sprint 11)

- [x] `audit.types.ts` — IAuditEntry (action 32 enum, entityType 21 enum, changes field-level diff), IAuditChange, ACTION_META, ENTITY_LABEL
- [x] `audit.mock.ts` — generator sinh từ ORDERS (create + transition), REFUNDS, VOUCHER_BATCHES, USERS, ROLES assign, LEAVE_REQUESTS decide, INCIDENTS event, TRANSFERS, STOCK_TAKES, REVIEWS, TICKETS, INQUIRIES, CATEGORIES, BRANDS, PRODUCTS, login events
- [x] `AuditStore` — entries, countByAction, countByEntity, filterBy({search, actorId, action, entityType, tenantId, dateRange})
- [x] `core/auth/audit.guard.ts` — functional `auditGuard: CanActivateFn` check `canViewWholeCompany || role-auditor`, redirect `/forbidden`
- [x] Mở rộng `notification.mock.ts` từ 8 lên ~60 noti sinh theo các event domain (order, ticket, review, leave, incident, campaign, voucher, system, IAM, stock) + meta map 7 type

### Pages (Sprint 11)

- [x] `/audit` — DataTable 7-col (mã code/occurredAt/actor+ip/action badge/entityType+label/tenant/view btn) + 5 filter (search + action + entity + tenant + dateRange) + pagination 25/page (11a)
- [x] Detail Drawer khi click row audit — info DescriptionList + field-level diff before/after side-by-side + metadata key-value (11a)
- [x] `/notifications` — list standalone với 3-stat card + 4 filter (search/type/read state/date range) + bulk select (checkbox) + mark-read/archive bulk (11b)
- [x] Topbar dropdown thêm footer link "Xem tất cả thông báo →" trỏ `/notifications` (11b)

### Routing + Layout

- [x] Thay placeholder `/audit` bằng route thật + apply `auditGuard`
- [x] Thêm route `/notifications` ở `app.routes.ts` (lazy)
- [x] Sidebar group "Hệ thống" thêm entry "Thông báo" với icon Lucide Bell
...
| 2026-05-21 | Sprint 11 | Audit 11a: types (32 action × 21 entityType × IAuditChange field-level diff + ACTION_META badge map + ENTITY_LABEL). audit.mock generator sinh ~600 entry deterministic từ existing stores (ORDERS create+transition, REFUNDS, CAMPAIGNS, PROMOTIONS, VOUCHER_BATCHES, BRANDS, PRODUCTS create/update, CATEGORIES, TRANSFERS create+transition, STOCK_TAKES create+complete, REVIEWS moderate, TICKETS transition, INQUIRIES reply, USERS create + ROLES assign, LEAVE_REQUESTS submit+decide, INCIDENTS events, ~3 login events per user). AuditStore với entries/countByAction/countByEntity/filterBy hỗ trợ search + 5 filter. core/auth/audit.guard.ts functional CanActivateFn check canViewWholeCompany                                                                                                                                                                                                                                                                                                                                                                                                  |                                                                                                                                                                                                                                                | role-auditor, redirect `/forbidden`. Page `/audit` DataTable 7-col (mã, time, actor+ip, action badge, entity, tenant, view) + 5 filter + pagination 25/page. Detail Drawer khi click code: DescriptionList info + field-level diff before/after side-by-side (rose/emerald) + metadata key-value. Xoá audit-placeholder. |
| 2026-05-21 | Sprint 11 | Notification 11b: mở rộng NotificationType từ 4 lên 7 (thêm marketing/hr/crm) + NOTIFICATION_TYPE_META badge+dot maps. INotification.archived field. notification.mock generator sinh ~60 entry tổng cộng từ ORDERS, TICKETS, REVIEWS pending, LEAVE_REQUESTS pending, INCIDENTS critical+high, CAMPAIGNS, VOUCHER_BATCHES + 6 system seeds + 4 IAM seeds + 6 stock seeds, sort desc theo createdAt. NotificationStore mở rộng: total/countByType/todayCount + markManyAsRead/archiveMany cho bulk action. Page `/notifications` standalone (route ở app.routes.ts lazy) với 3-stat card (Tổng/Chưa đọc/Hôm nay) + 4 filter + Checkbox bulk select + 2 bulk action (mark read/archive) + click row mark read + navigate. Sidebar "Hệ thống" thêm entry "Thông báo" với LucideBell. Topbar dropdown footer thay text bằng link "Xem tất cả thông báo →" trỏ /notifications. NOTI_ICON_MAP topbar mở rộng đủ 7 type (đổi LucideAlertTriangle deprecated → LucideTriangleAlert).                                                                                                                                  |
